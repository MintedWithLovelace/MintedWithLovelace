...
*Note: I'm aware that 777 is a top level, in this solution I'm suggesting using 777 as just the token name as a "friendly" reference that this is a token related to royalty setting. I think making it of type 777 would not work in this solution as it would be expected to have different structure and would have different expectations, for example it would be expected to have the same exact policy id of the target policy we are setting royalty for...which is not how this solution works as it is designed to be capable of setting royalties on any policy id derived from the same keyHash as this one and thus uses a different validation technique and process as outlined below. In this solution we need a more complex metadata structure to contain this tokens' policy script data and the target policy script data and royalty data as well.  So all reference to 777 throughout this document is referencing the Name of this token, not the type.*
